...
describe('v0.38 migrations v80 + v81 (smoke)', () => {
  test('v80 drops takes_kind_check; takes accepts arbitrary kind values', async () => {
    // After migration v80, INSERTing a non-default kind (e.g. 'finding') should succeed.
    ...
    // Pre-v80 this would have failed the CHECK constraint.
    await engine.executeRaw(
  ...
  test('v81 adds eval_candidates.schema_pack_per_source column', async () => {
  ...
  test('v81 accepts JSONB shape for per-source snapshot', async () => {
    const snapshot = {
      default: {
